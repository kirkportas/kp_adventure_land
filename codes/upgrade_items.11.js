...
function compound_items(){
    ...
    for (let itemname of COMPOUNDABLE) {
        let max_level_compound = COMPOUNDABLE_LEVELS[itemname];
        let lvl = 0;
        for (; lvl < max_level_compound; lvl++) {
            var item_idxs = locate_items_of_level(itemname, lvl);
            var count = item_idxs.length;
            if (count >= 3) {

                var grade = item_grade(character.items[item_idxs[0]]);
                if (grade >= 2 && !ALLOWED_RARE_COMPOUNDS.includes(itemname)) {
                    Logger.log("compound_item called for grade 2+. NOT SUPPORTED");
                    Logger.log(`name: ${itemname}`);
                    return;
                }

                let scrollname = "cscroll"+grade; //"cscroll0","cscroll1","cscroll2"
                var scroll_idx = locate_item(scrollname);
                if (scroll_idx < 0) {
                    buy(scrollname, 5);
                }

                Logger.log(`${itemname}: ${item_idxs}`);
                use_skill("massproduction");
                compound(item_idxs[0],item_idxs[1],item_idxs[2],scroll_idx);
            }
        }
