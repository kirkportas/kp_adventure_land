...
function _use(skill,target,extra_args) {
    /*enable target switching to last second*/
    targets[skill]=target;
    if (target) {
        DEBUGLOG && console.log("_use target: "+target);
        DEBUGLOG && console.log("_use target.length: "+target.length); 
    } else {
    ...
    DEBUGLOG && console.log("cooldownTime: "+cooldownTime);
    if (!cooldownTime) { 
        oldUse_skill(skill,target,extra_args); 
        return Promise.reject("No timer on this spell?");
    }

    if (!!cooldownTime && 
        timingsForAttacks[getCDName(skill)] === cooldownTime && 
        mssince(cooldownTime) < 0) {
            return Promise.reject("cooldown 0: "+skill+ " "+mssince(cooldownTime)); //if we already timed on the attack time
    }
    if (mssince(cooldownTime) < -700) {
        return Promise.reject("cooldown -700: "+skill+ " "+mssince(cooldownTime));
    ...
    DEBUGLOG && console.log("mssince(cooldownTime): "+mssince(cooldownTime));
...
// It didn't return a promise, unlike all the other cases.
function _use_skill(skill,target,extra_args) {
    if (skill === "attack") {
        DEBUGLOG && console.log("Returning oldAttack () ");
        return oldAttack(target);
    }

    DEBUGLOG && console.log("Returning oldUse_skill () ");
    return oldUse_skill(skill,target,extra_args);
    // return Promise.resolve(`Used skill: ${skill}`);
}
